...
- Geocode records: geocodes record addresses into the `geocoded_records` table with `uv run python -m dataprep.pipelines.geocode_records.cli` (args in [`dataprep/pipelines/geocode_records/cli.py`](dataprep/pipelines/geocode_records/cli.py)). Records the geocoder cannot resolve fall back to manual coordinates in [`data/geocode_overrides.csv`](data/geocode_overrides.csv), keyed by `record_number`.
